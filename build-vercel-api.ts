#!/usr/bin/env node
/**
 * Build script for Vercel API bundle (TypeScript)
 * Runs with tsx which supports path aliases natively
 */
...
(async () => {
  try {
    ...
    const result = await esbuild.build({
      ...
      // Don't use alias - tsx resolves TypeScript paths natively
      logLevel: 'info',
      sourcemap: false,
      // Mark heavy deps as external
      external: ['@trpc/server', '@trpc/server/adapters/express', 'express', 'pg', 'mysql2'],
    });
