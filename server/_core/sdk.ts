...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    try {
      ...
      // If DB is unreachable, we'll handle it below
      if (!((err as any)?.cause?.code === "ENOTFOUND" || (err as any)?.message?.includes("ENOTFOUND"))) {
    ...
    if (!user) {
      try {
        // If session has openId (local dev session), use it directly
        ...
        // If DB is unreachable, create a fallback user object for dev/admin
    ...
    try {
      ...
      // If DB is unreachable on update, still return the user
      if ((err as any)?.cause?.code === "ENOTFOUND" || (err as any)?.message?.includes("ENOTFOUND")) {
        console.log("[Auth] Database unreachable on user update, continuing with existing user");
      } else {
        throw err;
      }
    }
